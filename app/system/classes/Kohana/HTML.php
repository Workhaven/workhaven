...
class Kohana_HTML {
	...
	/**
         * Used for determine if html entities "li"/"a"/.. should be marked as 
         * active.Checks given controller/action, compares it to current url and
         * determines wheter the item is active.
	 * 
	 *  Examples:
         * 
	 *     echo HTML::active("user","settings", "active"); 
         *     -> if current page is user/settings "activate" is returned
         * 
	 *     echo HTML::active("user","settings"); 
         *     -> if current page is user/settings "class='active'" is returned
         * 
	 * @param   string $controller  controller from url
         * @param   string $action      action from url 
         * @param   string $ret_str     return string
	 * @return  string
	 */        
        public static function active($controller, $action = NULL, $ret_str = "class='active'") {           
            //$url_to_compare = $controller;
            //$url_part = lcfirst(Request::current()->controller());                         
            if(lcfirst($controller) == lcfirst(Request::current()->controller())){
                if(is_array($action)){
                    if(in_array(lcfirst(Request::current()->action()), $action) ) 
                        return " ".$ret_str;
                }else if($action == NULL){
                    return " ".$ret_str;
                } else {
                    $action = lcfirst($action);                      
                    if($action == lcfirst(Request::current()->action()))
                        return " ".$ret_str;                                        
                }                
            }             
            return "";
