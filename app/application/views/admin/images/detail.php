                ...
                <div class="media-body col-xs-12 col-sm-10 col-md-11 col-lg-11" id="comment<?=$comment->id?>">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <h4 class="media-heading"><?=$comment->user->email.$comment->author_visitor." • <span class='datetime'>".  Date::process($comment->created) . "</span>"?><?php if(Auth::instance()->logged_in("admin")){ ?><a href="/comments/delete/<?=$comment->id?>" class="confirm-modal-dialog" data-message="Do you really want to delete this?"><button type="button" class="close" aria-hidden="true">×</button></a><?php } ?></h4>
                            <p><?=nl2br($comment->text)?></p>                            
                            <?=HTML::anchor('#', __("Reply"), array("class" => "reply") )?>
                        </div>
                    </div>
                    <div class="media reply">                
                        <form method="post" action="<?= URL::site('/admin/images/addcomment/'.$image_id.'?replyfor='.$comment->id)?>">
                            <div class="col-xs-12 col-sm-2 col-md-1 col-lg-1">
                                <!-- Place for an image.  -->
                            </div>
...
<div class="container">    
    ...
    <div class="center-block image-detail">
        <img src="/images/projects/<?=Arr::get($data["values"], "project_id")?>/<?=Arr::get($data["values"], "filename")?><?=Arr::get($data, "image_revision")?>" alt="<?=Arr::get($data["values"], "description")?>" id='<?=Arr::get($data["values"], "id")?>' class="note" />
    </div>
    <?php if($data["values"]["description"]){ ?>
    <div class="well well-lg"><?=nl2br(Arr::get($data["values"], "description"))?></div>
    <?php } ?>
    <div style="display: none">
        <?php include Kohana::find_file('views', 'admin/images/edit') ?>
    </div>
    <br />
    <br />
</div>                    
...
<div id="discussion">
    <div class="container">
        <div class="header">
            ...
            <?=HTML::anchor('/admin/images/detail/'.Arr::get($data["values"], "id").'?refresh='.time().'#discussion', '<span class="glyphicon glyphicon-refresh pull-right"></span>')?>
        </div>
        <ul class="media-list">
            <li class="media">
                <form method="post" action="<?= URL::site('/admin/images/addcomment/'.Arr::get($data["values"], "id"))?>">
                    <div class="col-xs-12 col-sm-2 col-md-1 col-lg-1">
                        <!-- Place for an image.  -->
                    </div>
                    <div class="media-body col-xs-12 col-sm-10 col-md-11 col-lg-11">
